...
export class AnalyticsService {
    ...
    async getDashboardStats() {
        ...
        const totalResponsables = users.filter(u => u.role === 'responsableformation').length;
        const totalAdmins = users.filter(u => u.role === 'Admin').length;

        // Formation statuses
        const formationsActives = formations.filter(f => f.statut === 'active').length;
        const formationsInactives = formations.filter(f => f.statut === 'inactive').length;

        // Eleve statuses
        const elevesActifs = eleves.filter((e: any) => e.statut === 'actif').length;
        const elevesInactifs = eleves.filter((e: any) => e.statut === 'inactif').length;
        const elevesArchives = eleves.filter((e: any) => e.statut === 'archive').length;

        // Inscription statuses
        const inscriptionsByStatus = {
            en_cours: inscriptions.filter(i => i.statut_formation === 'en_cours').length,
            completee: inscriptions.filter(i => i.statut_formation === 'completee').length,
            abandonnee: inscriptions.filter(i => i.statut_formation === 'abandonnee').length,
        };

        // Presence stats
        const presencePresent = presences.filter((p: any) => p.statut === 'present' || p.present === true).length;
        const presenceAbsent = presences.filter((p: any) => p.statut === 'absent' || p.present === false).length;

        // Rates
        const tauxPresence = totalPresences > 0 ? Math.round((presencePresent / totalPresences) * 100) : 0;
        const tauxCompletion = totalInscriptions > 0 ? Math.round((inscriptionsByStatus.completee / totalInscriptions) * 100) : 0;
        const tauxAbandon = totalInscriptions > 0 ? Math.round((inscriptionsByStatus.abandonnee / totalInscriptions) * 100) : 0;

        // Users by role chart data
        const usersByRole = [
            { name: 'Formateurs', value: totalFormateurs, color: 'hsl(var(--primary))' },
            { name: 'Responsables', value: totalResponsables, color: 'hsl(var(--warning))' },
        ...
        const elevesParFormation = formations.map(f => {
            const formationInscriptions = inscriptions.filter(
                i => i.id_formation && (i.id_formation as any)._id?.toString() === f._id.toString()
            );
            return {
                name: f.nom_formation || 'Sans nom',
                eleves: formationInscriptions.length,
                enCours: formationInscriptions.filter(i => i.statut_formation === 'en_cours').length,
                completee: formationInscriptions.filter(i => i.statut_formation === 'completee').length,
                abandonnee: formationInscriptions.filter(i => i.statut_formation === 'abandonnee').length,
            };
        });
