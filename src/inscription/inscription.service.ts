...
export class InscriptionService {
    ...
    async create(createInscriptionDto: CreateInscriptionDto): Promise<Inscription> {
        ...
        const existing = await this.inscriptionModel.findOne({
            id_eleve: eleveId,
            id_formation: formationId,
        }).exec();

        if (existing) {
            throw new ConflictException('Eleve is already enrolled in this formation');
        }
    ...
    async updateStatus(id: string, updateStatusDto: UpdateStatusDto): Promise<Inscription> {
