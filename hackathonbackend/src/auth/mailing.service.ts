...
export class MailingService {
  ...
  constructor() {
    ...
    this.logger.debug(
      `Configuration: ${smtpConfig.host}:${smtpConfig.port} (secure: ${smtpConfig.secure})`,
    );

    // Vérifier que les credentials sont configurés
    if (!smtpConfig.auth.user || !smtpConfig.auth.pass) {
      this.logger.warn(
        "⚠️ ATTENTION: Variables d'environnement SMTP_USER ou SMTP_PASSWORD non configurées!",
      );
    }
  ...
  async sendAccountCreation(
    ...
    try {
      ...
      this.logger.log(
        `✅ Email d'inscription envoyé à ${email} (ID: ${info.messageId})`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erreur lors de l\'envoi de l\'email d'inscription:`,
        error.message,
      );
      throw new Error(
        `Impossible d\'envoyer l\'email de création: ${error.message}`,
      );
    }
  }
  ...
  async sendForgetPassword(
    ...
    try {
      ...
      const info = await this.transporter.sendMail(mailOptions);
      this.logger.log(
        `✅ Email de réinitialisation envoyé à ${email} (ID: ${info.messageId})`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erreur lors de l\'envoi du code à ${email}:`,
        error.message,
      );
      throw new Error(
  ...
  async sendFormatorDelayNotification(
    ...
    const mailOptions = {
      ...
      html: `
                ...
                <p>Vous êtes <strong>en retard de ${minutesDeLay} minutes</strong> pour la séance suivante:</p>
    ...
    try {
      ...
      const info = await this.transporter.sendMail(mailOptions);
      this.logger.log(
        `✅ Email de notification de retard envoyé à ${email} (ID: ${info.messageId})`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Erreur lors de l\'envoi de la notification de retard à ${email}:`,
        error.message,
      );
      throw new Error(
        `Impossible d\'envoyer la notification de retard: ${error.message}`,
      );
    }
  }
}
