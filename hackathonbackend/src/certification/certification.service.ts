...
export class CertificationService {
  ...
  async create(
    ...
    const inscription = await this.inscriptionModel
      .findById(createCertificationDto.id_inscription)
      .exec();
    if (!inscription) {
    ...
    const existing = await this.certificationModel
      .findOne({ id_inscription: createCertificationDto.id_inscription })
      .exec();
    if (existing) {
      throw new ConflictException(
        'Certificate already exists for this inscription',
      );
    }
  ...
  async findAll(): Promise<Certification[]> {
    return this.certificationModel
      ...
      .populate({
        path: 'id_inscription',
        populate: [{ path: 'id_eleve' }, { path: 'id_formation' }],
      })
      .exec();
  }
  ...
  async generatePdf(id: string, res: Response): Promise<void> {
    ...
    doc
      .fontSize(20)
      .font('Helvetica-Bold')
      .text(`${formation.nom_formation}`, { align: 'center' });

    // Date
    doc.moveDown(2);
    doc
      .fontSize(12)
      .text(`Délivré le : ${cert.date_delivrance.toLocaleDateString()}`, {
        align: 'center',
      });
    ...
    doc
      .fontSize(10)
      .text(`N° Certificat : ${cert.numero_certificat}`, { align: 'center' });
