...
export function ManagerDashboard() {
  ...
  const handleAddEleve = async () => {
    if (!newEleve.nom.trim() || !newEleve.prenom.trim()) { toast.error(t('managerDashboard.nameFirstNameRequired')); return; }
    setAddingEleve(true);
    try {
      const c = await elevesService.create(newEleve, avatarFile || undefined);
      setEleves(p => [...p, c]);
      toast.success(t('managerDashboard.studentCreated') + ': ' + c.prenom + ' ' + c.nom);
  ...
  const handleSaveEleve = async () => {
    ...
    try {
      ...
      toast.success(t('managerDashboard.studentUpdated')); setEditingEleve(null);
      setEditAvatarFile(null); setEditAvatarPreview(null);
    } catch (e: any) { toast.error(e.message || t('common.error')); }
    finally { setSavingEleve(false); }
  };
  const handleDeleteEleve = async (id: string) => {
    try { await elevesService.remove(id); setEleves(p => p.filter(e => e._id !== id)); toast.success(t('managerDashboard.studentDeleted')); }
    catch (e: any) { toast.error(e.message || t('common.error')); }
  };
  ...
  const handleCreateFormation = async () => {
    if (!newFormation.nom_formation.trim()) { toast.error(t('managerDashboard.formationNameRequired')); return; }
    ...
    try {
      ...
      toast.success(t('managerDashboard.formationCreated') + ': ' + c.nom_formation);
  ...
  const handleSaveFormation = async () => {
    ...
    try {
      ...
      toast.success(t('managerDashboard.formationUpdated')); setEditingFormation(null);
    } catch (e: any) { toast.error(e.message || t('common.error')); }
    finally { setSavingFormation(false); }
  };
  const handleDeleteFormation = async (id: string) => {
    try { await formationService.remove(id); setFormations(p => p.filter(f => f._id !== id)); toast.success(t('managerDashboard.formationDeleted')); }
    catch (e: any) { toast.error(e.message || t('common.error')); }
  };
  ...
  const handleToggleNiveau = async (niveauId: string, currentStatut: boolean) => {
    try {
      ...
      toast.success(t('managerDashboard.sessionUpdated'));
    } catch (e: any) { toast.error(e.message || t('common.error')); }
  };
  ...
  const handleAddInscription = async () => {
    if (!newInsc.id_eleve || !newInsc.id_formation) { toast.error(t('managerDashboard.selectStudentAndFormation')); return; }
    setAddingInsc(true);
    try {
      await inscriptionService.create(newInsc);
      const updated = await inscriptionService.findAll();
      setInscriptions(updated);
      toast.success(t('managerDashboard.inscriptionCreated'));
  ...
  const handleUpdateInscStatus = async (id: string, statut: string) => {
    try {
      ...
      const updated = await inscriptionService.findAll();
      setInscriptions(updated);
      toast.success(t('managerDashboard.statusUpdated'));
    } catch (e: any) { toast.error(e.message || t('common.error')); }
  };
  ...
  const handleLoadPresences = async (seanceId: string) => {
    ...
    catch (e: any) { toast.error(e.message || t('common.error')); setPresences([]); }
  ...
  // ===== CHART DATA =====
  const inscByStatus = [
    { name: 'En cours', value: inProgressInsc.length, color: '#0f4c81' },
  ...
  const JOURS = [t('days.mon'), t('days.tue'), t('days.wed'), t('days.thu'), t('days.fri'), t('days.sat'), t('days.sun')];
  ...
  if (loading) {
    return (<div className="flex min-h-screen items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /><span className="ml-3 text-lg">{t('common.loading')}</span></div>);
  }
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
          ...
          <TabsContent value="dashboard" className="space-y-6">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader><CardTitle>{t('managerDashboard.inscriptionsByStatus')}</CardTitle></CardHeader>
                <CardContent>
                  {inscriptions.length > 0 ? (
                    <div>
                      <ResponsiveContainer width="100%" height={250}>
                        <PieChart>
                          ...
                          <Tooltip 
                            ...
                            formatter={(value, name, props) => [`${value} inscription(s)`, props.payload.name]}
                          />
                          <Legend 
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-2 text-xs" role="region" aria-label="Inscription status summary">
                        {inscByStatus.map((item) => (
                          <div key={item.name} className="p-2 bg-muted rounded text-center">
                  ...
                  ) : <p className="text-center text-muted-foreground py-8">{t('managerDashboard.noData')}</p>}
                </CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>{t('managerDashboard.enrolledByFormation')}</CardTitle></CardHeader>
                <CardContent>
                    ...
                    <div>
                      <ResponsiveContainer width="100%" height={250}>
                        <BarChart data={formChartData}>
                          ...
                          <YAxis 
                            tick={{ fill: 'var(--foreground)', fontSize: 12 }}
                            label={{ value: 'Inscrits', angle: -90, position: 'insideLeft' }}
                          />
                          <Tooltip 
                            contentStyle={{backgroundColor: 'var(--card)', border: '1px solid var(--border)', borderRadius: '0.5rem'}}
                            labelStyle={{color: 'var(--foreground)'}}
                            formatter={(value) => [`${value} élève(s)`, 'Inscriptions']}
                          />
                          <Bar dataKey="inscrits" fill="#0f4c81" radius={[8, 8, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                      <div className="mt-4 p-3 bg-muted rounded text-sm" role="region" aria-label="Formation enrollment summary">
                        <div className="flex items-center justify-between">
                          <span className="text-muted-foreground">Total inscriptions:</span>
                          <span className="font-semibold text-foreground">{formChartData.reduce((a, b) => a + b.inscrits, 0)} élèves</span>
                        </div>
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-muted-foreground">Formations:</span>
                          <span className="font-semibold text-foreground">{formChartData.length}</span>
                        </div>
                      </div>
                    </div>
                  ) : <p className="text-center text-muted-foreground py-8">{t('managerDashboard.noFormation')}</p>}
                </CardContent>
              </Card>
              ...
              <Card className="lg:col-span-2">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5" /> {t('managerDashboard.sessionCalendar')}</CardTitle>
                      <CardDescription className="mt-1">{allSeances.length} {t('managerDashboard.scheduledSessions')}</CardDescription>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="sm" className="text-xs ml-1" onClick={() => { const d = new Date(); setCalendarMonth(new Date(d.getFullYear(), d.getMonth(), 1)); }}>{t('managerDashboard.today')}</Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
              ...
              {/* AVANCEMENT */}
              <Card className="lg:col-span-2">
                <CardHeader><CardTitle>{t('managerDashboard.progressByFormation')}</CardTitle><CardDescription>{t('managerDashboard.levelsAndSessionsDesc')}</CardDescription></CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    ...
                    {formations.length === 0 && <p className="text-center text-muted-foreground py-4">{t('managerDashboard.noFormation')}</p>}
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          <TabsContent value="eleves" className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-xl font-semibold">{t('common.eleves')} ({eleves.length})</h2>
              <div className="flex gap-2">
                <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input type="search" placeholder={t('common.search')} value={searchEleve} onChange={(e) => setSearchEleve(e.target.value)} className="pl-10 w-64" /></div>
                <Dialog open={isAddEleveOpen} onOpenChange={setIsAddEleveOpen}>
                  <DialogTrigger asChild><Button className="gap-2"><UserPlus className="h-4 w-4" /> {t('common.add')}</Button></DialogTrigger>
                  <DialogContent>
                    <DialogHeader><DialogTitle>{t('managerDashboard.newStudent')}</DialogTitle><DialogDescription>{t('managerDashboard.fillInfo')}</DialogDescription></DialogHeader>
                    <div className="space-y-3">
                      <div className="flex justify-center">
                        <div className="relative cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
                          ...
                          <p className="text-xs text-center text-muted-foreground mt-1">{t('managerDashboard.photo')}</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1"><Label>{t('managerDashboard.firstNameRequired')}</Label><Input value={newEleve.prenom} onChange={(e) => setNewEleve(p => ({...p, prenom: e.target.value}))} disabled={addingEleve} /></div>
                        <div className="space-y-1"><Label>{t('managerDashboard.lastNameRequired')}</Label><Input value={newEleve.nom} onChange={(e) => setNewEleve(p => ({...p, nom: e.target.value}))} disabled={addingEleve} /></div>
                      </div>
                      <div className="space-y-1"><Label>{t('common.email')}</Label><Input type="email" value={newEleve.email} onChange={(e) => setNewEleve(p => ({...p, email: e.target.value}))} disabled={addingEleve} /></div>
                      <div className="space-y-1"><Label>{t('common.phone')}</Label><Input value={newEleve.telephone} onChange={(e) => setNewEleve(p => ({...p, telephone: e.target.value}))} disabled={addingEleve} /></div>
                      <div className="space-y-1"><Label>{t('managerDashboard.birthDate')}</Label><Input type="date" value={newEleve.date_naissance} onChange={(e) => setNewEleve(p => ({...p, date_naissance: e.target.value}))} disabled={addingEleve} /></div>
                      <div className="space-y-1"><Label>{t('common.address')}</Label><Input value={newEleve.adresse} onChange={(e) => setNewEleve(p => ({...p, adresse: e.target.value}))} disabled={addingEleve} /></div>
                    </div>
                    <DialogFooter>
                      <Button variant="outline" onClick={() => setIsAddEleveOpen(false)}>{t('common.cancel')}</Button>
                      <Button onClick={handleAddEleve} disabled={addingEleve}>{addingEleve ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />...</> : t('common.create')}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            <Dialog open={!!editingEleve} onOpenChange={(o) => { if (!o) { setEditingEleve(null); setEditAvatarFile(null); setEditAvatarPreview(null); } }}>
              <DialogContent>
                <DialogHeader><DialogTitle>{t('managerDashboard.editStudent')}</DialogTitle></DialogHeader>
                <div className="space-y-3">
                  <div className="flex justify-center">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1"><Label>{t('common.firstName')}</Label><Input value={editEleveData.prenom} onChange={(e) => setEditEleveData(p => ({...p, prenom: e.target.value}))} disabled={savingEleve} /></div>
                    <div className="space-y-1"><Label>{t('common.name')}</Label><Input value={editEleveData.nom} onChange={(e) => setEditEleveData(p => ({...p, nom: e.target.value}))} disabled={savingEleve} /></div>
                  </div>
                  <div className="space-y-1"><Label>{t('common.email')}</Label><Input type="email" value={editEleveData.email} onChange={(e) => setEditEleveData(p => ({...p, email: e.target.value}))} disabled={savingEleve} /></div>
                  <div className="space-y-1"><Label>{t('common.phone')}</Label><Input value={editEleveData.telephone} onChange={(e) => setEditEleveData(p => ({...p, telephone: e.target.value}))} disabled={savingEleve} /></div>
                  <div className="space-y-1"><Label>{t('common.address')}</Label><Input value={editEleveData.adresse} onChange={(e) => setEditEleveData(p => ({...p, adresse: e.target.value}))} disabled={savingEleve} /></div>
                  <div className="space-y-1"><Label>{t('common.status')}</Label>
                    <Select value={editEleveData.statut} onValueChange={(v) => setEditEleveData(p => ({...p, statut: v}))}>
                      ...
                      <SelectContent><SelectItem value="actif">{t('common.active')}</SelectItem><SelectItem value="inactif">{t('common.inactive')}</SelectItem><SelectItem value="archive">Archive</SelectItem></SelectContent>
                    </Select>
                  </div>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setEditingEleve(null)}>{t('common.cancel')}</Button>
                  <Button onClick={handleSaveEleve} disabled={savingEleve}>{savingEleve ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />...</> : t('common.save')}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={!!inspectEleve} onOpenChange={(o) => { if (!o) { setInspectEleve(null); setInspectPresences([]); } }}>
              <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                <DialogHeader><DialogTitle>{t('managerDashboard.studentFile')}</DialogTitle><DialogDescription>{t('managerDashboard.studentFileDesc')}</DialogDescription></DialogHeader>
                {loadingInspect ? <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin" /></div> : (
                  <div className="space-y-6">
                    ...
                    <div>
                      <h4 className="font-medium mb-2">{t('managerDashboard.information')}</h4>
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <span className="text-muted-foreground">{t('common.email')}:</span><span>{inspectEleve?.email || '-'}</span>
                        <span className="text-muted-foreground">{t('common.phone')}:</span><span>{inspectEleve?.telephone || '-'}</span>
                        <span className="text-muted-foreground">{t('common.address')}:</span><span>{inspectEleve?.adresse || '-'}</span>
                        <span className="text-muted-foreground">{t('managerDashboard.registeredOn')}:</span><span>{inspectEleve?.date_inscription ? new Date(inspectEleve.date_inscription).toLocaleDateString('fr-FR') : '-'}</span>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-medium mb-2">{t('common.inscriptions')} & {t('managerDashboard.presenceRate')}</h4>
                      {(() => {
                        const eleveInscs = inscriptions.filter(i => {
                          const eId = typeof i.id_eleve === 'object' ? (i.id_eleve as Eleve)._id : i.id_eleve;
                          return eId === inspectEleve?._id;
                        });
                        if (eleveInscs.length === 0) return <p className="text-sm text-muted-foreground">{t('managerDashboard.noInscription')}</p>;
                          ...
                          return (
                            <div key={insc._id} className="border border-border rounded-md p-3 mb-2">
                              ...
                              <div className="mt-2">
                                ...
                                <p className="text-[10px] text-muted-foreground mt-0.5">{t('managerDashboard.presenceRate')} ({presentCount}/{totalSeances}) — {t('managerDashboard.requiredForCert')}</p>
                              </div>
                              {alreadyCertified ? (
                                <div className="mt-2 flex items-center gap-1 text-xs text-green-600"><Award className="h-3 w-3" /> {t('managerDashboard.certified')}</div>
                              ) : isEligible ? (
                                ...
                                }}><Award className="h-4 w-4" /> {t('managerDashboard.generateCert')}</Button>
                              ) : (
                                <p className="mt-2 text-[10px] text-muted-foreground">{t('managerDashboard.insufficientPresence')}</p>
                              )}
                            </div>
                          );
                        });
                    ...
                    <div>
                      <h4 className="font-medium mb-2">{t('common.certifications')}</h4>
                      {(() => {
                        ...
                        if (eleveCerts.length === 0) return <p className="text-sm text-muted-foreground">{t('managerDashboard.noCertification')}</p>;
                        return eleveCerts.map(cert => (
                          <div key={cert._id} className="border border-border rounded-md p-3 mb-2 flex items-center justify-between">
                            <div><Award className="h-4 w-4 inline mr-1 text-primary" /><span className="text-sm font-medium">{cert.numero_certificat}</span><p className="text-xs text-muted-foreground">{t('managerDashboard.issuedOn')} {cert.date_delivrance ? new Date(cert.date_delivrance).toLocaleDateString('fr-FR') : '-'}</p></div>
            ...
            <Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full">
              <thead className="border-b bg-muted/50"><tr>
                <th className="px-4 py-3 text-left font-medium">{t('common.eleves')}</th>
                ...
                <th className="px-4 py-3 text-left font-medium">{t('managerDashboard.registeredOn')}</th>
              ...
              <tbody className="divide-y divide-border">
                ...
                {filteredEleves.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">{t('managerDashboard.noStudent')}</td></tr>}
              </tbody>
          ...
          <TabsContent value="formations" className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-xl font-semibold">{t('common.formations')} ({formations.length})</h2>
              <div className="flex gap-2 flex-wrap">
                <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input type="search" placeholder={t('managerDashboard.searchFormation')} value={searchFormation} onChange={(e) => setSearchFormation(e.target.value)} className="pl-10 w-56" /></div>
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                  <SelectTrigger className="w-28"><Filter className="h-4 w-4 mr-1" /><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="all">{t('managerDashboard.allFilter')}</SelectItem><SelectItem value="active">{t('managerDashboard.activeFilter')}</SelectItem><SelectItem value="inactive">{t('managerDashboard.inactiveFilter')}</SelectItem></SelectContent>
                </Select>
                <Dialog open={isCreateFormOpen} onOpenChange={setIsCreateFormOpen}>
                  <DialogTrigger asChild><Button className="gap-2"><Plus className="h-4 w-4" /> {t('common.create')}</Button></DialogTrigger>
                  <DialogContent>
                    <DialogHeader><DialogTitle>{t('managerDashboard.newFormation')}</DialogTitle><DialogDescription>{t('managerDashboard.newFormationDesc')}</DialogDescription></DialogHeader>
                    <div className="space-y-3">
                      <div className="space-y-1"><Label>{t('managerDashboard.formationNameRequired')}</Label><Input value={newFormation.nom_formation} onChange={(e) => setNewFormation(p => ({...p, nom_formation: e.target.value}))} disabled={creatingFormation} /></div>
                      <div className="space-y-1"><Label>{t('common.description')}</Label><Textarea value={newFormation.description} rows={3} onChange={(e) => setNewFormation(p => ({...p, description: e.target.value}))} disabled={creatingFormation} /></div>
                      <div className="space-y-1"><Label>{t('common.status')}</Label>
                    ...
                    <DialogFooter>
                      <Button variant="outline" onClick={() => setIsCreateFormOpen(false)}>{t('common.cancel')}</Button>
                      <Button onClick={handleCreateFormation} disabled={creatingFormation}>{creatingFormation ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />...</> : t('common.create')}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            <Dialog open={!!editingFormation} onOpenChange={(o) => { if (!o) setEditingFormation(null); }}>
              <DialogContent>
                <DialogHeader><DialogTitle>{t('managerDashboard.editFormation')}</DialogTitle></DialogHeader>
                <div className="space-y-3">
                  <div className="space-y-1"><Label>{t('common.name')}</Label><Input value={editFormData.nom_formation} onChange={(e) => setEditFormData(p => ({...p, nom_formation: e.target.value}))} disabled={savingFormation} /></div>
                  <div className="space-y-1"><Label>{t('common.description')}</Label><Textarea value={editFormData.description} rows={3} onChange={(e) => setEditFormData(p => ({...p, description: e.target.value}))} disabled={savingFormation} /></div>
                  <div className="space-y-1"><Label>{t('common.status')}</Label>
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setEditingFormation(null)}>{t('common.cancel')}</Button>
                  <Button onClick={handleSaveFormation} disabled={savingFormation}>{savingFormation ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />...</> : t('common.save')}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={!!enrollFormationId} onOpenChange={(o) => { if (!o) { setEnrollFormationId(null); setEnrollSelectedEleves([]); } }}>
              <DialogContent className="max-w-md max-h-[70vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>{t('managerDashboard.enrollStudents')}</DialogTitle>
                  <DialogDescription>{t('managerDashboard.enrollStudentsDesc')}</DialogDescription>
                </DialogHeader>
                {enrollFormationId && (() => {
                  const uninscribed = getUninscribedEleves(enrollFormationId);
                  if (uninscribed.length === 0) return <p className="text-sm text-muted-foreground py-4">{t('managerDashboard.allStudentsEnrolled')}</p>;
                  return (
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => { setEnrollFormationId(null); setEnrollSelectedEleves([]); }}>{t('common.cancel')}</Button>
            ...
            {filteredFormations.length === 0 ? (
              <Card><CardContent className="p-12 text-center"><BookOpen className="h-12 w-12 mx-auto mb-4 text-muted-foreground" /><p className="text-muted-foreground">{t('managerDashboard.noFormation')}</p></CardContent></Card>
            ) : (
              <div className="space-y-4">
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground mb-2">{f.description || t('managerDashboard.noDesc')}</p>
                            <div className="flex gap-4 text-sm text-muted-foreground">
                              <span>{activeNivs}/{niveaux.length} {t('managerDashboard.activeLevels')}</span>
                              <span>{seancesCount} seances</span>
                              <span>{formInsc.length} {t('managerDashboard.enrolled')}</span>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            <Button variant="outline" size="sm" className="gap-1" onClick={() => handleOpenEnroll(f._id)}><UserPlus className="h-4 w-4" /> {t('managerDashboard.enrollStudents')}</Button>
          ...
          <TabsContent value="seances" className="space-y-6">
            <h2 className="text-xl font-semibold">{t('managerDashboard.levelsAndSessions')}</h2>
            <p className="text-muted-foreground">{t('managerDashboard.levelsAndSessionsDesc')}</p>
            {formations.length === 0 ? (
              <Card><CardContent className="p-12 text-center"><Calendar className="h-12 w-12 mx-auto mb-4 text-muted-foreground" /><p className="text-muted-foreground">{t('managerDashboard.noFormation')}</p></CardContent></Card>
            ) : (
              <div className="space-y-3">
                {formations.map(f => {
                  const detail = formationDetails[f._id];
                              ...
                              <div key={niveau._id} className="mt-4">
                                <div className="flex items-center justify-between mb-2 bg-muted/30 rounded-lg px-3 py-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Badge variant={niveau.statut ? 'default' : 'outline'}>{niveau.statut ? t('common.active') : t('common.inactive')}</Badge>
                                  </div>
                                  <Button size="sm" variant="ghost" onClick={() => handleToggleNiveau(niveau._id, niveau.statut)}>
                                    {niveau.statut ? t('managerDashboard.disable') : t('managerDashboard.enable')}
                                  </Button>
                                </div>
                                      ...
                                      {editingSeance === seance._id ? (
                                        <div className="space-y-2">
                                          <div className="grid grid-cols-2 gap-2">
                                            <div><Label className="text-xs">{t('managerDashboard.title2')}</Label><Input value={seanceEdit.titre} onChange={(e) => setSeanceEdit(p => ({...p, titre: e.target.value}))} className="h-8 text-sm" /></div>
                                            <div><Label className="text-xs">{t('managerDashboard.plannedDate')}</Label><Input type="date" value={seanceEdit.date_prevue} onChange={(e) => setSeanceEdit(p => ({...p, date_prevue: e.target.value}))} className="h-8 text-sm" /></div>
                                            <div><Label className="text-xs">{t('managerDashboard.startTime')}</Label><Input type="time" value={seanceEdit.heure_debut} onChange={(e) => setSeanceEdit(p => ({...p, heure_debut: e.target.value}))} className="h-8 text-sm" /></div>
                                            <div><Label className="text-xs">{t('managerDashboard.endTime')}</Label><Input type="time" value={seanceEdit.heure_fin} onChange={(e) => setSeanceEdit(p => ({...p, heure_fin: e.target.value}))} className="h-8 text-sm" /></div>
                                          </div>
                                          <div className="flex gap-2 justify-end">
                                            <Button size="sm" variant="ghost" onClick={() => setEditingSeance(null)}><X className="h-4 w-4" /></Button>
                                            <Button size="sm" onClick={() => handleSaveSeance(seance._id, f._id)}><Save className="h-4 w-4 mr-1" /> {t('common.save')}</Button>
                                          </div>
                                        </div>
                                      ) : (
                                        <div className="flex items-center justify-between">
          ...
          <TabsContent value="inscriptions" className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-xl font-semibold">{t('common.inscriptions')} ({inscriptions.length})</h2>
              <div className="flex gap-2 flex-wrap">
                <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input type="search" placeholder={t('managerDashboard.searchStudent')} value={inscFilterEleve} onChange={(e) => setInscFilterEleve(e.target.value)} className="pl-10 w-56" /></div>
                <Select value={inscFilterFormation} onValueChange={setInscFilterFormation}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">{t('managerDashboard.allFormations')}</SelectItem>
                ...
                <Select value={inscFilterStatus} onValueChange={setInscFilterStatus}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">{t('managerDashboard.allStatuses')}</SelectItem>
                    <SelectItem value="en_cours">{t('common.inProgress')}</SelectItem>
                    <SelectItem value="completee">{t('common.completed')}</SelectItem>
                    <SelectItem value="abandonnee">{t('common.abandoned')}</SelectItem>
                  </SelectContent>
                </Select>
                <Dialog open={isAddInscOpen} onOpenChange={setIsAddInscOpen}>
                  <DialogTrigger asChild><Button className="gap-2"><Plus className="h-4 w-4" /> {t('managerDashboard.enrollStudents')}</Button></DialogTrigger>
                  <DialogContent>
                    <DialogHeader><DialogTitle>{t('managerDashboard.newInscription')}</DialogTitle><DialogDescription>{t('managerDashboard.newInscriptionDesc')}</DialogDescription></DialogHeader>
                    <div className="space-y-3">
                      <div className="space-y-1"><Label>{t('common.eleves')}</Label>
                        <Select value={newInsc.id_eleve} onValueChange={(v) => setNewInsc(p => ({...p, id_eleve: v}))}>
                          <SelectTrigger><SelectValue placeholder={t('managerDashboard.select')} /></SelectTrigger>
                          <SelectContent>{eleves.map(e => (<SelectItem key={e._id} value={e._id}>{e.prenom} {e.nom}</SelectItem>))}</SelectContent>
                        </Select>
                      </div>
                      <div className="space-y-1"><Label>{t('common.formations')}</Label>
                    ...
                    <DialogFooter>
                      <Button variant="outline" onClick={() => setIsAddInscOpen(false)}>{t('common.cancel')}</Button>
                      <Button onClick={handleAddInscription} disabled={addingInsc}>{addingInsc ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />...</> : t('managerDashboard.enrollStudents')}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            {filteredInscriptions.length === 0 ? (
              <Card><CardContent className="p-12 text-center"><FileText className="h-12 w-12 mx-auto mb-4 text-muted-foreground" /><p className="text-muted-foreground">{t('managerDashboard.noInscription')}</p></CardContent></Card>
            ) : (
              <div className="space-y-3">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="flex gap-2">
                            {insc.statut_formation === 'en_cours' && (<>
                              <Button size="sm" variant="outline" className="gap-1 text-destructive border-destructive" onClick={() => handleUpdateInscStatus(insc._id, 'abandonnee')}><XCircle className="h-4 w-4" /> {t('managerDashboard.abandon')}</Button>
                              <Button size="sm" className="gap-1" onClick={() => handleUpdateInscStatus(insc._id, 'completee')}><CheckCircle2 className="h-4 w-4" /> {t('managerDashboard.complete')}</Button>
                            </>)}
                          </div>
                        </div>
                      </CardContent>
          ...
          <TabsContent value="certifications" className="space-y-6">
            <h2 className="text-xl font-semibold">{t('common.certifications')}</h2>

            {/* Existing certificates */}
            {certifications.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-muted-foreground">{t('managerDashboard.issuedCertificates')} ({certifications.length})</h3>
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">N° {cert.numero_certificat} | {t('managerDashboard.issuedOn')} {cert.date_delivrance ? new Date(cert.date_delivrance).toLocaleDateString('fr-FR') : '-'} par {cert.delivre_par}</p>
                          </div>
                        </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('managerDashboard.generateCertificates')}</CardTitle>
                <CardDescription>{t('managerDashboard.eligibleStudents')} (taux de presence &ge; 75% des seances de la formation)</CardDescription>
              </CardHeader>
              <CardContent>
                {(() => {
